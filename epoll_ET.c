...
int main(){
    ...
    event.events = EPOLLIN | EPOLLET; // server 边缘触发
    ...
    while(1){  
        ...
        for(int i=0;i<nfds;i++){
            ...
            if(events[i].data.fd == server_fd){
                ...
                while(1){  // 使用while循环一次将所有的server_fd全部acccept
                    struct sockaddr_in client_addr;
                    socklen_t addrlen = sizeof(client_addr);
                    int client_fd = accept(server_fd,
                                    (struct sockaddr *)&client_addr,
                                    &addrlen);  // 使用while循环一次将所有的server_fd全部acccept
            ...
            //process data
                ...
                if(events[i].events & EPOLLIN){
                    ...
                    while(1){ // 重点!:  continue read until emptied the receive buffer.
